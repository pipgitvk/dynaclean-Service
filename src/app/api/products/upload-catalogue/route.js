// src/app/api/installation-videos/upload-catalogue/route.js
import { NextResponse } from "next/server";
import { getDbConnection } from "@/lib/db";
import fs from "fs/promises";
import path from "path";

const UPLOAD_DIR = path.join(process.cwd(), "public", "product_documents", "catalogues");
const ALLOWED = {
  "application/pdf": ".pdf",
...
export async function POST(request) {
  try {
    ...
    const filename = `${base}_catalogue_${Date.now()}${ext}`;
    const filePath = path.join(UPLOAD_DIR, filename);

    const buffer = Buffer.from(await file.arrayBuffer());
    await fs.writeFile(filePath, buffer);

    const publicPath = `/product_documents/catalogues/${filename}`;

    const conn = await getDbConnection();
    let sql = ""; let params = [];
    if (id) { sql = "UPDATE products_list SET catalogue_link = ? WHERE id = ?"; params = [publicPath, id]; }
    else { sql = "UPDATE products_list SET catalogue_link = ? WHERE item_code = ?"; params = [publicPath, item_code]; }
